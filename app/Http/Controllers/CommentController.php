...
class CommentController extends Controller
{
    public function index(Request $request)
    {
        ...
        $comments = Comment::join('users', 'users.id', '=', 'comments.user_id')->select('comments.*', 'users.firstname', 'users.lastname', 'users.profile_image', 'users.username', 'users.provider_id')->where('comments.post_id', $request->get('post_id'))->skip($request->get('skip'))->take(6)->orderBy('created_at', 'desc')->get();
        return $comments;
    }
